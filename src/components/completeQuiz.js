...
export default function CompletePage(){
    ...
    const {arrC} = location.state

    const openQuiz = (e, title, questions, attempted) => {
        navigate('/quizPage', {state: {title, questions, attempted}})
    }
        ...
        <div className="completed-container">
            {
                arrC.map((item, index) => {
                    ...
                    return(
                        <div className="title-cards" key={index} onClick={
                            (e) => openQuiz(e, item[titles], item[questions], item[attempted])
                        }>
                            {item[titles]}
                        </div>
                    )
